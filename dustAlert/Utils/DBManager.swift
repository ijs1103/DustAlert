...
final class DBManager {
    ...
    func getLikeLocationFromDB() -> [String] {
        let userDefaults = UserDefaults.standard
        if let likeLocation = userDefaults.object(forKey: Const.Id.favoriteDust) as? Data {
            if let decodedLikeLocation = try? JSONDecoder().decode(Set<String>.self, from: likeLocation) {
        ...
        if let likeLocation = userDefaults.object(forKey: Const.Id.favoriteDust) as? Data {
            if var decodedLikeLocation = try? JSONDecoder().decode(Set<String>.self, from: likeLocation) {
                ...
                if let encodedLikeLocation = try? JSONEncoder().encode(decodedLikeLocation) {
                    userDefaults.set(encodedLikeLocation, forKey: Const.Id.favoriteDust)
                }
            }
        } else {
            if let encoded = try? JSONEncoder().encode([location]) {
                userDefaults.set(encoded, forKey: Const.Id.favoriteDust)
            }
        }
    }
    // location이 Dust 타입의 식별자이다
